...
class Dishes extends Product {
   constructor(protected id: number, public name: string, public price: number, public description: string, public inStock: number, public isFlat?: boolean) {
      super(id, name, price, description, inStock);
      this.Init();
   }
...
function CheckSmart(flag: boolean) {
   ...
   if (flag) {
      for (let i = 0; i < this.productList.length; i++)
         if (productList[i] instanceof Clocks && (<Clocks>productList[i]).isSmart) (<Clocks>productList[i]).Init();
   }
   else {
      for (let i = 0; i < this.productList.length; i++)
         productList[i].Init();
...
function CheckFlat(flag: boolean) {
   document.getElementById('rowts').innerHTML = "";
   if (flag) {
      for (let i = 0; i < this.productList.length; i++)
         if (productList[i] instanceof Dishes && (<Dishes>productList[i]).isFlat) (<Dishes>productList[i]).Init();
   }
   else {
      for (let i = 0; i < this.productList.length; i++)
         productList[i].Init();
